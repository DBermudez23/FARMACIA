...
function Login() {
  ...
  const { setToken, backendURL } = useContext(VendedorContext);
  ...
  const onSubmitHandler = async (event) => {
    ...
    setCargando(true);
    try {
      const {data} = await axios.post(backendURL + '/api/vendedor/login', {mail, contrasena});
      if (data.success) {
        localStorage.setItem('token', data.token);
        setToken(data.token);
        navigate('/vendedor-panel-principal');
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div className="bg-white shadow-md rounded-xl px-10 py-12 w-full max-w-sm">
        ...
        <p className="text-center text-sm text-gray-600 mb-6">VENDEDOR</p>
